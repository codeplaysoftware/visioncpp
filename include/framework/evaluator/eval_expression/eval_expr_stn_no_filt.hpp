...
/// \file eval_expr_stn_no_filt.hpp
/// \brief This file contains the specialisation of the EvalExpr
/// for StnFilt( stencil node without filter).

#ifndef VISIONCPP_INCLUDE_FRAMEWORK_EVALUATOR_EVAL_EXPRESSION_EVAL_EXPR_STN_NO_FILT_HPP_
#define VISIONCPP_INCLUDE_FRAMEWORK_EVALUATOR_EVAL_EXPRESSION_EVAL_EXPR_STN_NO_FILT_HPP_

namespace visioncpp {
namespace internal {
/// \brief Partial specialisation of the EvalExpr when the expression is
/// an StnNoFilt(stencil without filter operation) expression.
...
struct EvalExpr<StnNoFilt<C_OP, Halo_T, Halo_L, Halo_B, Halo_R, RHS, Cols, Rows,
                          LfType, LVL>,
                Loc, Params...> {
