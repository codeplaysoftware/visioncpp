...
void run_test(QUEUE &q, DATA data, int i) {
  ...
  std::shared_ptr<float> ret_val(
      new float[common::singleton::DataSet::Instance().m_height *
...

  {
    ...
    auto return_node = visioncpp::terminal<
        visioncpp::pixel::F32C3, common::singleton::DataSet::m_width,
        common::singleton::DataSet::m_height, visioncpp::memory_type::Buffer2D>(
        ret_val.get());
    auto node = visioncpp::point_operation<visioncpp::OP_BGRToRGB>(data);
    auto node2 = visioncpp::point_operation<visioncpp::OP_U8C3ToF32C3>(node);

    // assign data from node to return_node
    auto assign_node = visioncpp::assign(return_node, node2);
    // 4) execute pipe
