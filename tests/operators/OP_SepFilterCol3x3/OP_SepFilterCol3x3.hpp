...
void run_test(QUEUE &q, DATA data, int i) {
  ...
  {
    float filter_array[3] = {1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0};
    // 3) define graph
    ...
    auto filter_col =
        visioncpp::terminal<float, 3, 1, visioncpp::memory_type::Buffer2D,
                            visioncpp::scope::Constant>(filter_array);
    auto filter_row =
        visioncpp::terminal<float, 1, 3, visioncpp::memory_type::Buffer2D,
                            visioncpp::scope::Constant>(filter_array);
    ...
    auto node3 = visioncpp::neighbour_operation<visioncpp::OP_SepFilterRow>(
        node2, filter_row);
    auto node4 = visioncpp::point_operation<visioncpp::OP_RGBToCVBGR>(node3);

    // assign data from node to return_node
    auto assign_node = visioncpp::assign(return_node, node4);
    // 4) execute pipe
